...
from voicereport.diarization import run_diarization
...
async def analyze(
    ...
    # AUDIO + USER INFORMATION LOG
    try:
        logger.info("[VOICEREPORT MAIN] audio.filename=%s, content_type=%s, user_id=%s",
        ...
        if user_profile_json:
            ...
            logger.info("[VOICEREPORT MAIN] user_profile_json.len=%d, preview=%s", bytelen, preview)
        else:
            logger.info("[VOICEREPORT MAIN] user_profile_json is EMPTY")
    except Exception:
        pass
    ...
    try:
        try:
            ...
            logger.info("[VOICEREPORT MAIN] saved tmp_path=%s, bytes=%d", tmp_path, fsize)
        except Exception:
            logger.info("[VOICEREPORT MAIN] saved tmp_path=%s (size check failed)", tmp_path)
        ...
        if user_profile_json:
            try:
                ...
                logger.info("[VOICEREPORT MAIN] keys=%s", list(user_info.keys()))
                logger.info("[VOICEREPORT MAIN] preview child_age=%s, preferred_tone=%s",
                            user_info.get("child_age"), user_info.get("preferred_tone"))
            except Exception as e:
                logger.warning("[VOICEREPORT MAIN] parse error: %s", e)
        else:
            logger.warning("[VOICEREPORT MAIN] user_profile_json missing")

        # STT
        try:
            stt_result = run_stt(tmp_path)
            stt_segments = stt_result.get("segments", []) if isinstance(stt_result, dict) else stt_result
            logger.info("[VOICEREPORT MAIN] segments.count=%d", len(stt_segments) if stt_segments else 0)
        except Exception as e:
            logger.error("[VOICEREPORT MAIN] STT failed: %s", e, exc_info=True)
            raise HTTPException(status_code=500, detail="STT_FAILED")

        # Diarization
        diar_segments: List[Dict[str, Any]] = []
        if SKIP_DIARIZATION:
            logger.info("[VOICEREPORT MAIN] skipped by env (SKIP_DIARIZATION=1)")
        else:
            try:
                diar_segments = run_diarization(tmp_path)
                logger.info("[VOICEREPORT MAIN] ok, n_segments=%d", len(diar_segments))
            except Exception as e:
                logger.error("[VOICEREPORT MAIN] DIAR failed: %s", e, exc_info=True)
                raise HTTPException(status_code=500, detail="DIARIZATION_FAILED")

        # MAPPING
        try:
            turns = map_speaker_segments(stt_segments, diar_segments)
            logger.info("[VOICEREPORT MAIN] turns=%d", len(turns) if turns else 0)
        except Exception as e:
            logger.error("[VOICEREPORT MAIN] MAP failed: %s", e, exc_info=True)
            raise HTTPException(status_code=500, detail="MAPPING_FAILED")

        # 대화 텍스트 생성
        dialogue = _build_dialogue_text(turns)
        logger.info("[VOICEREPORT MAIN] len_chars=%d, lines=%d",
                    len(dialogue or ""), (dialogue or "").count("\n") + (1 if dialogue else 0))

        # PROMPT + LLM 호출
        try:
            prompt = build_prompt(dialogue, user_info)
            logger.info("[VOICEREPORT MAIN] PROMPT / len_chars=%d, user_keys=%s", len(prompt), list(user_info.keys()))
        except Exception as e:
            logger.error("[VOICEREPORT MAIN] PROMPT build failed: %s", e, exc_info=True)
            raise HTTPException(status_code=500, detail="PROMPT_BUILD_FAILED")

        try:
            model_out = request_claude(prompt)
            logger.info("[VOICEREPORT MAIN] CLAUDE keys=%s", list(model_out.keys()) if isinstance(model_out, dict) else type(model_out))
        except Exception as e:
            logger.error("[VOICEREPORT MAIN] LLM failed: %s", e, exc_info=True)
        ...
        if dur is None:
            ...
            logger.info("[VOICEREPORT MAIN] duration.fallback=%s", dur)
        ...
        logger.info(
            "[VOICEREPORT MAIN] subTitle=%s, day=%s, has_freq=%s, has_expr=%s, timeline_len=%s",
            dto.subTitle, dto.day,
        ...
        logger.error("[VOICEREPORT MAIN] analysis failed: %s", e, exc_info=True)
